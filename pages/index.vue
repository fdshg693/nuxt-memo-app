<template>
    ...
    <NuxtLink to="sql/allTables" class="
           inline-block
           px-8 py-4
           font-semibold text-white
           bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500
           ...
           transform transition
           hover:scale-105 hover:brightness-110
           focus:outline-none focus:ring-4 focus:ring-purple-300 mb-8">SQLで使うテーブルの一覧</NuxtLink>
    <NuxtLink to="sql/explanation" class="
           inline-block
           px-8 py-4
           font-semibold text-white
           bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500
           ...
           transform transition
           hover:scale-105 hover:brightness-110
           focus:outline-none focus:ring-4 focus:ring-purple-300 mb-8">SQL文の説明</NuxtLink>
            ...
            <div v-for="(subgenreGroup, subgenre) in genreGroup" :key="subgenre" class="mb-2 ml-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    <NuxtLink v-for="question in subgenreGroup" :key="question.id" :to="`sql/${question.id}`"
                        class="px-4 py-2 rounded bg-purple-100 hover:bg-purple-200 text-purple-800 font-medium transition text-center">
                        SQL問題{{ question.id }}
                    </NuxtLink>
                </div>
            </div>
